...
exports.getPlateGratingsToleranceById = asyncHandler (async (req, res) => {
  const plateGratingsToleranceById = await db.dopuskiPlo4iRe6etki.findOne ({
    attributes: [
      'id',
      'plateGridName',
      'cardNumber',
...
exports.createPlateGratingsTolerances = asyncHandler (async (req, res) => {
  const newPlateGratingsTolerances = await db.dopuskiPlo4iRe6etki.create ({
    plateGridName: req.body.plateGridName,
    cardNumber: req.body.cardNumber,
    nominal: req.body.nominal,
    nominalMin1: req.body.nominalMin1,
  ...
  if (newPlateGratingsTolerances) {
    res.json (newPlateGratingsTolerances);
  } else {
    res.status (404);
    throw new Error ('Ooops...Something went wrong!');
  }
