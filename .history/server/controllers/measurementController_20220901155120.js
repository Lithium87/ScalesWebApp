...
exports.applyFilters = asyncHandler (async (req, res) => {
  ...
  const folteredMeasurementsPerScale = await sequelize.query (
    'SELECT `materialType`, `operatorName`, `weight`, `density`, `mixer`, `byrkalo`, `penetration`, DATE(`createdAt`) AS `data` FROM `Measurements` AS `Measurement` WHERE `Measurement`.`scaleId` = :scaleId AND `Measurement`.`materialType` = :materialType AND `Measurement`.`operatorName` = :operatorName AND (`Measurement`.`data` >= :fromDate AND `Measurement`.`data` <= :toDate ',
    {
      replacements: {
