...
exports.applyFilters = asyncHandler (async (req, res) => {
  ...
  const filteredMeasurementsPerScale = await db.Measurement.findAll ({
    attributes: {
      include: [
        literal (`(
          SELECT scaleId, materialType, operatorName, date(createdAt) as data, time(createdAt) as vreme, weight, density, mixer, byrkalo, penetration
          WHERE scaleId = ? AND materialType = ? AND operatorName = ? AND data >= ? AND data <= ?
        )`),
