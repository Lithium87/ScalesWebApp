...
const {Op, QueryTypes, query} = require ('sequelize');
...
exports.applyFilters = asyncHandler (async (req, res) => {
  ...
  const filteredMeasurementsPerScale = (sequelize) => await sequelize.query (
    'SELECT scaleId, materialType, operatorName, weight, time(createdAt) as time, date(createdAt) as date, density, mixer, byrkalo, penetration FROM Measurements WHERE scaleId = :scaleId AND materialType = :materialType AND operatorName = :operatorName AND date BETWEEN(:date)',
    {
      replacements: {
        scaleId: req.params.id,
        materialType: filters.material,
